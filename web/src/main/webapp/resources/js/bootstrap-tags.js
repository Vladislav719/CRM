...
(function ($) {
    (function () {
        ...
        jQuery(function () {
            $.tags = function (element, options) {
                ...
                this.caseInsensitive || (this.caseInsensitive = true);
                this.readOnlyEmptyMessage || (this.readOnlyEmptyMessage = "No tags to display...");
                this.maxNumTags || (this.maxNumTags = -1);
                this.addSpec = options.addSpec || (this.addSpec = null);
                this.addSkill = options.addSkill || (this.addSkill = null);
                this.removeSpec = options.removeSpec || (this.removeSpec = null);
                this.removeSkill = options.removeSkill || (this.removeSkill = null);
                ...
                this.removeTagClicked = function (e) {
                    if (e.currentTarget.tagName === "A") {
                        var indexDeleteTag = $("span", e.currentTarget.parentElement).data('id');
                        var tags = $.grep(_this.tagsArray, function (value, i) {
                            if (indexDeleteTag == value.id) {
                                return value;
                            }
                        });
                        _this.removeTag(tags[0]);
                ...
                this.removeTag = function (tag) {
                    if (_this.tagsArray.indexOf(tag) > -1) {
                        ...
                        if (_this.removeSkill != null)
                            _this.removeSkill(tag);
                        if (_this.removeSpec != null)
                            _this.removeSpec(tag);
                ...
                this.addTag = function (tag) {
                    ...
                    if (_this.canAddByRestriction(tag) && !_this.hasTag(tag) && tag.name.length > 0 &&
                        _this.canAddByExclusion(tag) && _this.canAddByMaxNum()) {
                        if (_this.beforeAddingTag(tag) === false) {
                            return;
                        }
                        associatedContent = _this.definePopover(tag.name);
                        _this.popoverArray.push(associatedContent || null);
                        _this.tagsArray.push(tag);
                        //_this.afterAddingTag(tag.name);
                        _this.renderTags();
                        if (!_this.canAddByMaxNum()) {
                            _this.disableInput();
                        }
                        if (_this.addSpec != null)
                            _this.addSpec(tag);
                        if (_this.addSkill != null)
                            _this.addSkill(tag);
                    }
                ...
                this.keyDownHandler = function (e) {
                    ...
                    function hasTag(tag) {
                        return $.grep(options.tagsArray, function (value, i) {
                            if (tag.toUpperCase() == value.name.toUpperCase())
                                return true;
                        });
                        return false;
                    }
                    ...
                    switch (k) {
                        case 13:
                            ...
                            if (typeof tag == "object") {
                                ...
                                if (!(_this.tagsArray.some(function (element, index, array) {
                                        return tag.toUpperCase() == element.name.toUpperCase();
                                    }))) {
                                    var tags = $.grep(_this.suggestions, function (value, i) {
                                        if (tag.toUpperCase() == value.name.toUpperCase()) {
                                            return value;
                                        }
                                    });
                                    if (tags.length > 0) {
                                        ...
                                        if (!options.suggestOnClick) {
                                            var a = function (tag) {
                                                if (!(typeof tag == "object")) {
                                                    $.ajax({
                                                        url: "/ajax/add/new/skill",
                                                        type: "POST",
                                                        data: {"tag": tag},
                                                        success: (function (id) {
                                                            tag = {"id": id, "name": tag}
                ...
                this.getSuggestions = function (str, overrideLengthCheck) {
                    ...
                    $.each(this.suggestions, function (i, suggestion) {
                        var suggestionVal;
                        suggestionVal = _this.caseInsensitive ? suggestion.name.substring(0, str.length).toLowerCase() : suggestion.name.substring(0, str.length);
                        if (!(_this.tagsArray.some(function (element, index, array) {
                                return element.id == suggestion.id;
                            })) && _this.tagsArray.indexOf(suggestion) < 0 && suggestionVal === str && (str.length > 0 || overrideLengthCheck)) {
                ...
                this.renderTags = function () {
                    ...
                    $.each(_this.tagsArray, function (i, tag) {
                        ...
                        if (_this.displayPopovers) {
                            _this.initializePopoverFor(tag, _this.tagsArray[i].name, _this.popoverArray[i]);
                        }
    ...
    (function () {
        ...
        Tags.Templates["3"].tag = function (options) {
            ...
            return "<div class='tag label " + options.tagClass + " " + options.tagSize + "' " + (options.isPopover ? "rel='popover'" : "") + "> " +
                "   <span data-id='" + options.tag.id + "'>" + Tags.Helpers.addPadding(options.tag.name, 2, options.isReadOnly) + "</span>  " +
                "  " + (options.isReadOnly ? "" : "<a><i class='remove glyphicon glyphicon-remove-sign glyphicon-white' /></a>") + "  </div>";
        };
    }).call(this);
    (function () {
        var _base;
    ...
    (function () {
        ...
        Tags.Templates.shared.tags_suggestion = function (options) {
            ...
            return "<li data-id='" + options.suggestion.id + "' class='tags-suggestion'>" + options.suggestion.name + "</li>";
        };
    }).call(this);
    (function () {
        window.Tags || (window.Tags = {});
